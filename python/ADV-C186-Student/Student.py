...
firebase = firebase.FirebaseApplication("https://encrypt-3c3d6-default-rtdb.firebaseio.com/", None)
...
def Getdata():
 ...
 get_friends_data = firebase.get('/', your_friend_code)
 if (get_friends_data !=None):
     ...
     final_message = original.decode("utf-8")
     print(final_message)
     if (final_message not in lastvalue):
         print(final_message)
         message_text.insert(END, final_message+"\n")
         lastvalue = final_message
...
def enterRoom():
    ...
    your_friend_code = friends_code_entry.get()
